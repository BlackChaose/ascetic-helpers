...
foreach ($flags as $pictures) {
    foreach ($pictures as $name => $path) {
        ...
        //$flags[$index]["name_cyr"]=$result;
        //todo добавь в бд
        //todo сгенерируй список стран - переведи  и добавь через бд скриптом, кириллические названия
    }
    $index++;
}
...
try {
    ...
    try {
        $pdo = new PDO('mysql:host=localhost;dbname=reestr', 'php', 'php', $opt);
